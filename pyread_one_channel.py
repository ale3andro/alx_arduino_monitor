...
try:
    ...
    print("The update interval for free account is 15 seconds")
except IOError:
...
def alx_thread_fnc(event, filename, port, baudrate, upload):
    ...
    data.update({"Δεδομένα": [ ["Ημερομηνία", "Ώρα", "Τιμή"] ]})
    curValue0 = curValue1 = ''
    ...
    while True:
        ...
        try:   
            curValue = float(packet)
        except ValueError:
            print(f"Το πακέτο {packet} δεν είναι πραγματικός αριθμός! Attempting a graceful exit, please wait...")
            exit(-1)

        data['Δεδομένα'].append([getDate(), getTime(), curValue])
        save_data("Καταγραφή_" + filename + ".ods", data)
            
        if (upload): # Αποστολή δεδομένων στο thingspeak
            full_url = url + "&field1=" + str(curValue)
            try:
